...
namespace EdFi.Ods.Common.Serialization
{
    ...
    public class ExtensionsConverter : JsonConverter
    {
        ...
        public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
        {
            ...
            {
                string extensionName = ((string) reader.Value).ToMixedCase();

                string extensionObjectTypeName = GetExtensionClassAssemblyQualifiedName(extensionName);
                ...
                if (TryActivateObject(serializer, extensionObjectTypeName, out extensionObject))
                {
                    // Use the serializer to populate the extension object
                    serializer.Populate(reader, extensionObject);

                    // Assign the extension value
                    extensionDictionary[extensionName] = extensionObject;
                }
                else
                {
